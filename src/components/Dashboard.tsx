...
export function Dashboard({ 
  ...
  const [newExpenseTypeStatus, setNewExpenseTypeStatus] = useState<'fixo'|'variavel'>('variavel');
  ...
  const handleSaveExpense = async (e: React.FormEvent) => {
    ...
    for (let i = 0; i < (isReceita ? 1 : newExpenseInstallments); i++) {
        ...
        let desc = newExpenseTitle;
        if (!isReceita) {
           desc += newExpenseTypeStatus === 'fixo' ? ' [Fixo]' : ' [Variável]';
           if (newExpenseInstallments > 1) {
             desc += ` (${i+1}/${newExpenseInstallments})`;
           }
        }
        ...
        txsToInsert.push({
          ...
          description: desc,
          category: finalCategory,
          amount: isReceita ? amount : finalAmountPerInstallment,
    ...
    try {
      if (isTestMode) {
        const mockTxs = JSON.parse(localStorage.getItem('adielpay_mock_txs') || '[]');
        const toAdd = txsToInsert.map(tx => ({ ...tx, id: Math.random().toString() }));
        mockTxs.push(...toAdd);
      ...
      setNewExpenseTypeStatus('variavel');
    } catch (error) {
                ...
                <>
                  <div>
                    ...
                    <input 
                      ...
                      required
                    />
                    {newExpenseInstallments > 1 && (
                      <p className="text-xs text-slate-500 mt-1">
                        O valor será dividido em {newExpenseInstallments}x de {((parseFloat(newExpenseAmount) || 0) / newExpenseInstallments).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                      </p>
                    )}
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">Classificação de Gasto</label>
                    <div className="flex items-center gap-4">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input 
                          ...
                          name="expenseType" 
                          value="fixo" 
                          checked={newExpenseTypeStatus === 'fixo'}
                          onChange={() => setNewExpenseTypeStatus('fixo')}
                          className="w-4 h-4 text-blue-600 border-slate-300 focus:ring-blue-500"
                        />
                        <span className="text-slate-700 text-sm">Gasto Fixo</span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input 
                          type="radio" 
                          name="expenseType" 
                          value="variavel" 
                          checked={newExpenseTypeStatus === 'variavel'}
                          onChange={() => setNewExpenseTypeStatus('variavel')}
                          className="w-4 h-4 text-blue-600 border-slate-300 focus:ring-blue-500"
                        />
                        <span className="text-slate-700 text-sm">Gasto Variável</span>
                      </label>
                    </div>
                  </div>
                </>
